...
export const getCategory = () => { //  отправляем запрос на сервер чтоб получить спсиок категорий для фильтра на галвной станице
    ...
    return fetch(`${API_URI}api/category`).then(response => response.json()); //  отправляем запрос на сервер и получаем ответ response от сервера   и обрабатываем с помощью response.json()
};



// получаем  от сервера списко категрий для подвала
